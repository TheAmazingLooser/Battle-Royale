...
{
	// default health
	pChr->IncreaseHealth(g_Config.m_SvHealthStart);
	pChr->IncreaseArmor(g_Config.m_SvArmorStart);

	// give default weapons
	if(g_Config.m_SvHammerStart)
		pChr->GiveWeapon(WEAPON_HAMMER, -1);
	if(g_Config.m_SvGunStart)
		pChr->GiveWeapon(WEAPON_GUN, g_Config.m_SvGunStartAmmo);
	if(g_Config.m_SvShotgunStart)
		pChr->GiveWeapon(WEAPON_SHOTGUN, g_Config.m_SvShotgunStartAmmo);
	if(g_Config.m_SvGrenadeStart)
		pChr->GiveWeapon(WEAPON_GRENADE, g_Config.m_SvGrenadeStartAmmo);
	if(g_Config.m_SvLaserStart)
		pChr->GiveWeapon(WEAPON_RIFLE, g_Config.m_SvLaserStartAmmo);
	if(g_Config.m_SvNinjaStart)
		pChr->GiveNinja();

	if(!g_Config.m_SvHammerStart && !g_Config.m_SvGunStart && !g_Config.m_SvShotgunStart && !g_Config.m_SvGrenadeStart && !g_Config.m_SvLaserStart)
		pChr->GiveWeapon(WEAPON_HAMMER, -1);
}
...
void IGameController::Tick()
{
	...
	if(m_GameOverTick != -1 && m_IsStarted)
	{
